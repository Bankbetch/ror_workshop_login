class Api::AppController < ApplicationController
  ...
  def handle_400(exception)
    render json: { status: false, error: exception.message }, status: :bad_request and return
  end

  def handle_401(exception)
    render json: { status: false, error: exception.message }, status: :unauthorized and return
  end
end
